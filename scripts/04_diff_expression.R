...
heatmap_plot <- function(matrix_subset,
                         metadata,
                         contrast,
                         deg_table,
                         config) {
  ...
  headers <- list()
  p1 <- ggplot() +
    geom_segment(data = hc_segments, aes(
      x = x,
      xend = xend,
      y = y,
      yend = yend
  ...
  for (var in config$qc_variables) {
    p2 <- ggplot() +
      ...
      theme_void() +
      theme(
        plot.margin = margin(0, 0.4, 0.2, 0.4, "cm"),
  ...
  body_p <-
    patchwork::wrap_plots(headers, ncol = 1, heights = c(1, rep(0.5, length(
      config$qc_variables
    )), 8)) +
    patchwork::plot_layout(guides = "collect") +
    patchwork::plot_annotation(title = contrast) &
    theme(
      legend.position = "right",
      legend.box = "vertical",
...
extract_groups <- function(contrast) {
  groups <- strsplit(gsub("[()]", "", contrast), "[-+]")[[1]]
